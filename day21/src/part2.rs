...
pub fn solve_puzzle(file_name: &str, steps: u32) -> u32 {
    ...
    let start: (isize, isize) = map.iter().enumerate().find_map(|(y, row)| {
...
fn get_next_cells(map: &Vec<Vec<char>>, start: (isize, isize),
    ...
    let up_coord = (i-1, j);
    let up_value = map[(up_coord.0.rem_euclid(map.len() as isize)) as usize ][(up_coord.1.rem_euclid(map[0].len() as isize)) as usize];
    if up_value != '#'  {
        next_cells.push(up_coord);
    }
    let down_coord = (i+1, j);
    let down_value = map[(down_coord.0.rem_euclid(map.len() as isize)) as usize ][(down_coord.1.rem_euclid(map[0].len() as isize)) as usize];
    if down_value != '#'  {
        next_cells.push(down_coord);
    }
    let left_coord = (i, j-1);
    let left_value = map[(left_coord.0.rem_euclid(map.len() as isize)) as usize ][(left_coord.1.rem_euclid(map[0].len() as isize)) as usize];
    if left_value != '#'  {
        next_cells.push(left_coord);
    }
    let right_coord = (i, j+1);
    let right_value = map[(right_coord.0.rem_euclid(map.len() as isize)) as usize ][(right_coord.1.rem_euclid(map[0].len() as isize)) as usize];
    if right_value != '#'  {
